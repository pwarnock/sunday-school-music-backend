-- Migration: Add streak tracking tables and functions
-- Created: 2025-10-08 22:19:16
...
BEGIN
  ...
  SELECT 
    COALESCE(visit_count > 0, FALSE),
    COALESCE((messages_sent + songs_generated + chat_sessions) > 0, FALSE)
  ...
  SELECT 
    COALESCE(last_visit_date, '1900-01-01'::DATE), 
    COALESCE(last_activity_date, '1900-01-01'::DATE), 
    COALESCE(current_visit_streak, 0), 
    COALESCE(current_activity_streak, 0)
...
BEGIN
  ...
  INSERT INTO user_streaks (user_id, total_visits, total_activities)
  VALUES (
    p_user_id,
    CASE WHEN p_event_type = 'visit' THEN 1 ELSE 0 END,
    CASE WHEN p_event_type IN ('message_sent', 'song_generated', 'chat_started') THEN 1 ELSE 0 END
  )
  ON CONFLICT (user_id) DO UPDATE SET
    total_visits = user_streaks.total_visits + CASE WHEN p_event_type = 'visit' THEN 1 ELSE 0 END,
    total_activities = user_streaks.total_activities + CASE WHEN p_event_type IN ('message_sent', 'song_generated', 'chat_started') THEN 1 ELSE 0 END,
    updated_at = v_now;
